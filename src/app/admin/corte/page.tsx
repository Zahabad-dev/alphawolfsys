...
export default async function AdminCortePage() {
  ...
  const preciosParaForm = stock.map((s) => ({
    id: s.lote_id,
    etiqueta: `$${Number(s.precio_mxn).toFixed(2)}`,
  }));
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex flex-1 flex-col gap-6 p-4 sm:p-6">
        {preciosParaForm.length === 0 ? (
          <p className="rounded-2xl border border-white/10 bg-brand-gray2 p-4 text-sm text-brand-cream/70">
            El Almacén Central todavía no tiene precios. Ve a{" "}
            <a href="/admin/precios" className="text-brand-gold underline">
              Precios
            </a>{" "}
            y agrega uno seleccionando &quot;Almacén Central&quot; como sucursal.
          </p>
        ) : (
