...
export default async function PrecioQrPage({
  ...
  const { rows } = await query<PrecioRow>(
    `SELECT l.id, l.precio_mxn, l.qr_token, s.nombre AS sucursal_nombre
  ...
  return (
    <div className="flex min-h-screen flex-col items-center gap-6 bg-white p-8 text-black print:p-0">
      ...
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-black/10 p-8 print:border-0">
        <p className="text-lg font-semibold">{precio.sucursal_nombre}</p>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={qrDataUrl}
          alt={`QR $${Number(precio.precio_mxn).toFixed(2)}`}
          width={qr.size}
          height={qr.size}
          style={{ width: qr.size, height: qr.size }}
        />
        <p className="text-3xl font-bold">${Number(precio.precio_mxn).toFixed(2)} MXN</p>
      </div>
      <PrintButton />
    </div>
  );
